...
// *** START modified for BH@H interface ***
 int header_already_written = 0;
 {
   FILE *file;
   if (file = fopen(file_name_buffer, "r")) {
     fclose(file);

     header_already_written = 1;
   }
 }
// *** END modified for BH@H interface ***

const char *openMode;
//if (IO_TruncateOutputFiles(state.cgi.GH) == 1)
//   openMode = "w";
//else
openMode = "a"; // <- modified for BH@H interface
...
"        \"%s\"!"
 ...
 if(header_already_written == 1) return fileptr;  // <- modified for BH@H interface
